...
class AdminController:
    ...
    def registrar_permissions(body: PermissionCreate):
        ...
        if Permission.query.filter_by(nome=nova_permission.nome).first():
            return {"error": "Já existe uma permissão com este nome"}, 409
        try:
    ...
    def registrar_role():
        try:
            # pega o JSON e transforma num objeto RolesCreate
            body = RolesCreate(**request.get_json())

            # Agora body.nome e body.permissions funcionam corretamente
